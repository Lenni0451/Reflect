...
public class Constructors {
    ...
    private static final MethodHandle getDeclaredConstructors0 = FieldInitializer
            .attempt(() -> {
                if (OPENJ9_RUNTIME) {
            ...
            .require(() -> new MethodNotFoundException(Class.class.getName(), METHOD_Class_getDeclaredConstructors0, OPENJ9_RUNTIME ? "" : "boolean"));
    private static final Class<?> MemberName = FieldInitializer
            .attempt(() -> Classes.forName(CLASS_MemberName))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final Class<?> DirectMethodHandle_Constructor = FieldInitializer
            .attempt(() -> Classes.forName(CLASS_DirectMethodHandle_Constructor))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final Class<?> MethodHandleNatives_Constants = FieldInitializer
            .attempt(() -> Classes.forName(CLASS_MethodHandleNatives_Constants))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final MethodHandle getInitMethod = FieldInitializer
            .attempt(() -> TRUSTED_LOOKUP.findGetter(DirectMethodHandle_Constructor, "initMethod", MemberName))
            .map(m -> m.asType(MethodType.methodType(Object.class, MethodHandle.class)))
            .ensure(() -> new FieldNotFoundException(DirectMethodHandle_Constructor.getName(), "initMethod"))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final MethodHandle getFlags = FieldInitializer
            .attempt(() -> TRUSTED_LOOKUP.findGetter(MemberName, FIELD_MemberName_flags, int.class))
            .map(m -> m.asType(MethodType.methodType(int.class, Object.class)))
            .ensure(() -> new FieldNotFoundException(MemberName.getName(), FIELD_MemberName_flags))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final MethodHandle setFlags = FieldInitializer
            .attempt(() -> TRUSTED_LOOKUP.findSetter(MemberName, FIELD_MemberName_flags, int.class))
            .map(m -> m.asType(MethodType.methodType(void.class, Object.class, int.class)))
            .ensure(() -> new FieldNotFoundException(MemberName.getName(), FIELD_MemberName_flags))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final Integer MN_IS_METHOD = FieldInitializer
            .attempt(() -> TRUSTED_LOOKUP.findStaticGetter(MethodHandleNatives_Constants, FIELD_MethodHandleNatives_Constants_MN_IS_METHOD, int.class))
            .map(m -> (int) m.invokeExact())
            .ensure(() -> new FieldNotFoundException(MethodHandleNatives_Constants.getName(), FIELD_MethodHandleNatives_Constants_MN_IS_METHOD))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final Integer MN_IS_CONSTRUCTOR = FieldInitializer
            .attempt(() -> TRUSTED_LOOKUP.findStaticGetter(MethodHandleNatives_Constants, FIELD_MethodHandleNatives_Constants_MN_IS_CONSTRUCTOR, int.class))
            .map(m -> (int) m.invokeExact())
            .ensure(() -> new FieldNotFoundException(MethodHandleNatives_Constants.getName(), FIELD_MethodHandleNatives_Constants_MN_IS_CONSTRUCTOR))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    private static final MethodHandle getDirectMethod = FieldInitializer
            .firstOf(
                    // Java 8
            ...
            .ensure(() -> new MethodNotFoundException(MethodHandles.Lookup.class.getName(), METHOD_MethodHandles_Lookup_getDirectMethod, "byte", Class.class.getName(), MemberName.getName(), MethodHandles.Lookup.class.getName()))
            .onlyIf(!OPENJ9_RUNTIME)
            .get();
    ...
    public static <T> Constructor<T>[] getDeclaredConstructors(final Class<T> clazz) {
        if (OPENJ9_RUNTIME) {
            return (Constructor<T>[]) getDeclaredConstructors0.invokeExact(clazz);
        } else {
            return (Constructor<T>[]) getDeclaredConstructors0.invokeExact(clazz, false);
        }
    }
