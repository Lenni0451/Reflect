...
public class Modules {
    ...
    private static final MethodHandle implAddExportsOrOpens = FieldInitializer
            .attempt(() -> Methods.getDeclaredMethod(Module.class, METHOD_Module_implAddExportsOrOpens, String.class, Module.class, boolean.class, boolean.class, boolean.class))
            .map(TRUSTED_LOOKUP::unreflect)
            .require(() -> new MethodNotFoundException(Module.class.getName(), METHOD_Module_implAddExportsOrOpens, String.class, Module.class, boolean.class, boolean.class, boolean.class));
    ...
    private static final MethodHandle implAddEnableNativeAccessToAllUnnamed = FieldInitializer
            .attempt(() -> Methods.getDeclaredMethod(Module.class, METHOD_Module_addEnableNativeAccessToAllUnnamed))
            .map(TRUSTED_LOOKUP::unreflect)
            .require(() -> new MethodNotFoundException(Module.class.getName(), METHOD_Module_addEnableNativeAccessToAllUnnamed));
    ...
    public static void openModule(final Class<?> clazz, final String pkg) {
        try {
            ...
            implAddExportsOrOpens.invoke(clazz.getModule(), pkg, everyone, true, true, true);
        } catch (Throwable t) {
            Sneaky.sneakyThrow(t); //TODO: Use @SneakyThrows once lombok supports Java 26
        }
    }
        ...
        try {
            ...
            for (Module module : ModuleLayer.boot().modules()) {
                for (String pkg : module.getPackages()) {
                    implAddExportsOrOpens.invoke(module, pkg, everyone, true, true, true);
                }
            }
        } catch (Throwable t) { //TODO: Use @SneakyThrows once lombok supports Java 26
    ...
    public static void enableNativeAccess(final Class<?> clazz) {
        try {
            ...
            Sneaky.sneakyThrow(e); //TODO: Use @SneakyThrows once lombok supports Java 26
        }
    }
